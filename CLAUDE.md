...
ローカルビルドの扱い・公証・自動起動・アップデート等の共通事項はベース側に詳しい。
...
LOCAL=1 bash Scripts/bundle.sh debug   # ローカル検証ビルド（本番と区別。下記「ローカルビルド」参照）
```

ローカルで動かして検証するときは `LOCAL=1 bash Scripts/bundle.sh debug` で生成した
`Keykun (Local).app` を `open` する（本番 `com.mtkg.keykun` と権限が衝突しない）。
...
開発中の検証ビルドは、配布版（`com.mtkg.keykun`）と**バンドルID・表示名・TCC 権限を分離**して動かす。
（共通方針は `../CLAUDE_base.md` のセクション7・8を参照。）

- **生成**: `LOCAL=1 bash Scripts/bundle.sh debug`。バンドルIDを `com.mtkg.keykun.local`、
  表示名を `Keykun (Local)` にする（`CFBundleExecutable` は `Keykun` のまま）。`Scripts/bundle.sh` が
  `PlistBuddy` で `CFBundleIdentifier`/`CFBundleName`/`CFBundleDisplayName` を差し替える。
- **「ローカル」表示**: アプリは `Bundle.main.bundleIdentifier` が `.local` で終わるかで `isLocalBuild` を判定
  （`StatusBarController`）。ローカルビルドはメニューバーアイコンに「ローカル」を併記し、メニュー先頭の
  バージョン項目にも「(ローカル)」を付けて本番と一目で区別する。
- **権限が別エントリ**: バンドルID が違うので、システム設定 ＞ アクセシビリティの権限一覧に本番とは
  **別エントリ**として並ぶ。ローカルビルドには**改めてアクセシビリティ権限を付与**する必要がある
  （本番の許可は引き継がれない）。
- **公証はされない**: 公証(notarization)は CI のリリースビルドのみ。ローカルビルドは**署名はされるが
  公証されない**。配布物と取り違えない。未公証のため quarantine が付くと起動を阻まれることがあり、
  必要なら `xattr -dr com.apple.quarantine "Keykun (Local).app"`。
- **自己更新は基底IDで判定**: `SelfUpdater` は bundle ID 検証を `.local` を除いた基底ID で行うため、
  ローカルビルドからも本番リリースへ更新できる（`SelfUpdater.baseBundleID`）。
