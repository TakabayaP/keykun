...
ターミナル限定の Command / Control 交換、Command-H の無効化を提供する。
...
swift build                  # ビルド
swift test                   # 全テスト
swift test --filter <Name>   # 個別テスト（例: ModifierTapDetectorTests）
swift run                    # 直接実行（開発時）
bash Scripts/bundle.sh       # .app バンドル生成（既定 release + Developer ID 署名）
AD_HOC=1 bash Scripts/bundle.sh   # 証明書が無い環境向けにアドホック署名
AD_HOC=1 bash Scripts/bundle.sh debug  # 証明書が無い場合のアドホック署名ビルド
```

検証するときは `bash Scripts/bundle.sh debug` で生成した `Keykun.app` を `open` する。
...
- **`KeykunCore`（ライブラリ / テスト対象）**: AppKit/CGEventTap に依存しないロジックとモデル。
  ...
  - `Settings` / `InputSwitchSettings` / `SlackEscapeSettings` / `CopyPasteSettings` /
    `TerminalModifierSwapSettings` / `CommandShortcutSettings` — 設定モデル。機能ごとにサブ構造体を持ち、
...
続いて `TerminalModifierSwapHandler` がターミナル向け修飾キーを交換する。`CopyPasteHandler` は交換後の
Ctrl-Shift-C/V をコピー・ペーストとして処理する。通常アプリの Command-H は `CommandHideHandler` が消費する。
...
- **イベントタップは1つを共有**する（`KeyEventTap`）。機能ごとに別タップを作らず `KeyEventHandler` を登録する。
  全ハンドラがイベントを観測（状態更新のため）し、消費は OR で決まる。複数ハンドラが同じイベントの消費を取り合うようになったら順序設計が必要。
